...
class UserService(ModelService):
    ...
    def new_user(self, username, password, email, describe=''):
        """
        创建用户
        :return: User
        """
        encrypted_password = security.generate_password_hash(password)
        user = User(username=username, password=encrypted_password, email=email, describe=describe)
        user.save()
        return user
    ...
    def authenticate(self, username, password):
        ...
        if user.password == security.generate_password_hash(password):
            return user
        else:
            return None
